...
def game():
    ...
    print(map.map_art(map.display_text_next_to_map(
        map.rewrite_spot(start_map, current_character["x_coordinate"], current_character["y_coordinate"], 2), menu_text,
        0)))
    while True:
        player_input = str(input("To play type \"start\":")).strip().lower()
        if player_input:
